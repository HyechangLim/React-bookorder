...
const CartProvider = (props) => {
    ...
    const addItemToCartHandler = (item) => {
        ...
        // //1. 기본으로 add할 경우
        // //기존에 cart에 등록된 item에 방금 새로 전달된 item 추가
        // const updatedItem = [
        //     ...cartState.items,
        //     item
        // ];
        ...
        let updatedItems;
        if(existingCartItem){ // 방금 새로 등록한 book(item)이 cart에 이미 존재할때
            
            const updatedItem={
                ...existingCartItem,
                amount : existingCartItemIndex.amount + item.amount,
            };
            updatedItems = [...cartState.items];
            updatedItems[existingCartItemIndex] = updatedItem;

        }else{ //방금 새로 등록한(item)이 cart에 존재하지 않을때
